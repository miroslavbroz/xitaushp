...
do i = 1, size(a,1)
  do j = 1, size(a,2)
    ...
    s = 0.d0
    do k = max(1, 2-i+size(psf,1)/2), min(size(psf,1), size(a,1)+1-i+size(psf,1)/2)
      do l = max(1, 2-j+size(psf,2)/2), min(size(psf,2), size(a,2)+1-j+size(psf,2)/2)

        s = s + a(i+k-size(psf,1)/2-1, j+l-size(psf,2)/2-1) * psf(k,l)
